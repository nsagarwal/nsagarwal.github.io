...
function loadMapData() {
  getMonthlyData().forEach(d => {
    d3.csv(import.meta.env.BASE_URL + `data/map/${d.month}.csv`).then(function(data) {
      mapData[d.month] = data;
    })
  })
}

d3.csv(import.meta.env.BASE_URL + "data/monthly.csv").then(function(data_1) {
  data_1.forEach(d => {
    d.active = +d.active;
    d.total = +d.total;
    d.index = +d.index;
...
// TODO : the last month is hard coded here but should be read in from the data
  d3.csv(import.meta.env.BASE_URL + "data/map/2026-03.csv").then(function(data_2) {
