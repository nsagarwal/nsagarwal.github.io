...
noUiSlider.create(graphSlider, {
  start: [0, MonthlyDataLength - 1],
  connect: true,
  range: { min: 0, max: MonthlyDataLength - 1 },
  step: 1,
  tooltips: false,
  format: {
...
export function updateSliderLabels() {
  ...
  const startLabel = formatMonthYear(monthlyData[start].month);
  const endLabel = formatMonthYear(monthlyData[end].month);
...
function loadNationalGraph() {
  ...
  d3.csv(import.meta.env.BASE_URL + "data/national.csv").then(function(data) {
    ...
    populateColumnCheckboxes();
    setSliders(0, MonthlyDataLength - 1);
    graphName = "National"
    updateGraph();
  });
}
...
export function updateGraph() {
  ...
  facilityList.forEach((code, i) => {
    name = `${facilityMap[code].name} (${facilityMap[code].state})`;
...
function getColor(key, index = 0) {
  ...
  if (key == "Midnight population") return "#0000FF";
  if (key == "24-hour population") return "#FF0000";
  if (key == "Book-ins") return "#007000";
  if (key == "Book-outs") return "#E69F00";
  return "black";
}
